...
func TestExecuteDyffAssertions(t *testing.T) {
	...
	t.Run("pass when files match (full render)", func(t *testing.T) {
		...
		golden := "/suite/golden.yaml"
		actual := "/out/render.yaml"
		content := []byte("apiVersion: v1\nkind: ConfigMap\nmetadata:\n  name: foo\n")
		require.NoError(t, afero.WriteFile(fs, golden, content, 0o644))
		require.NoError(t, afero.WriteFile(fs, actual, content, 0o644))
		...
		all := exec.executeAssertionsDyff(assertions)
		require.Len(t, all, 1)
	...
	t.Run("fail when files differ", func(t *testing.T) {
		...
		require.NoError(t, afero.WriteFile(fs, golden, []byte("expected: value\n"), 0o644))
		require.NoError(t, afero.WriteFile(fs, actual, []byte("actual: other\n"), 0o644))
	...
	t.Run("dyff respects bunt OFF (no ANSI)", func(t *testing.T) {
		bunt.SetColorSettings(bunt.OFF, bunt.OFF)
		defer bunt.SetColorSettings(bunt.AUTO, bunt.AUTO)
		...
		require.NoError(t, afero.WriteFile(fs, actual, []byte("actual: other\n"), 0o644))
		outputs := &engine.Outputs{Render: actual, Rendered: map[string]string{}}
		assertions := []api.AssertionGoldenFile{{Name: "dyff no color", Expected: "golden.yaml"}}
		exec := newAssertionExecutor(fs, outputs, false, testSuiteFile, expandPath, false)
		all := exec.executeAssertionsDyff(assertions)
		require.Len(t, all, 1)
		assert.Equal(t, engine.StatusFail(), all[0].Status)
		assert.NotContains(t, all[0].Message, "\033[", "dyff output should have no ANSI when bunt is OFF")
	})

	t.Run("dyff respects bunt ON (ANSI in output)", func(t *testing.T) {
		...
		require.NoError(t, afero.WriteFile(fs, actual, []byte("actual: other\n"), 0o644))
		outputs := &engine.Outputs{Render: actual, Rendered: map[string]string{}}
		assertions := []api.AssertionGoldenFile{{Name: "dyff with color", Expected: "golden.yaml"}}
		exec := newAssertionExecutor(fs, outputs, false, testSuiteFile, expandPath, false)
		all := exec.executeAssertionsDyff(assertions)
		require.Len(t, all, 1)
		assert.Equal(t, engine.StatusFail(), all[0].Status)
		assert.Contains(t, all[0].Message, "\033[", "dyff output should contain ANSI when bunt is ON")
	})
}
